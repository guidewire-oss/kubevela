	...
	attributes: {
		...
		status: healthPolicy: #"""
			ready: {
				...
				// Only read the object's conditions when a readyConditionType is set.
				// With none set (the backward-compatible default) leave conditions as
				// the empty default, so an object whose status.conditions is null or a
				// non-list shape cannot break the health evaluation.
