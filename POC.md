...
- [x] Re-execution: `status.workflows[].steps[].attempts`, `status.attempts`, `vela operation restart/resume/suspend [--step]`, a `Suspended` phase, and a `--default-operation-ttl-seconds`/`spec.ttlSecondsAfterFinished` GC window for terminal `Operation`s. No idempotency contract: deliberately rejected (any step, whatever its nature, can be rerun on request — the operator is trusted). `--failed-only`/dispatch-children re-run stays out of scope until Composition and Fan-out lands.
