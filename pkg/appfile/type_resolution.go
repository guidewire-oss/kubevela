...
func parseTypeRef(typeName string) (form int, module, apiVersion, name string, err error) {
	...
	case 2:
		if !apiVersionRe.MatchString(parts[0]) {
			return 0, "", "", "", fmt.Errorf(
...
// derivation is the same algorithm stampIdentity uses, so this requires no
// cluster call.
func resolveForm3(module, apiVersion, name string) string {
	return moduleNameTruncate(module + "-" + apiVersion + "-" + name)
}
...
func resolveForm1(ctx context.Context, cli client.Reader, typeName, name string, capType types.CapType) (string, error) {
	...
	// Not a legacy definition; try the module label fallback.
	matches, err := listModuleDefinitions(ctx, cli, capType, map[string]string{
		types.LabelDefinitionName: name,
	})
	if err != nil {
...
// moduleNameTruncate keeps a derived module definition name within the
// 253-character Kubernetes object-name limit, appending a stable 8-char hash
// suffix when needed. Uses the same algorithm as stampIdentity in render.go.
func moduleNameTruncate(name string) string {
	if len(name) <= maxModuleNameLen {
		return name
	}
	sum := sha256.Sum256([]byte(name))
	suffix := "-" + hex.EncodeToString(sum[:])[:8]
	return name[:maxModuleNameLen-len(suffix)] + suffix
}
