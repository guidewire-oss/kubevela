...
// rather than a cost control: a definition assembled from six places is not one
...
//
// A definition with no namespace is from a cluster-scoped install predating
// namespaced definitions, and is exempt.
func sameNamespace(kind, name, found, want string) error {
	if want == "" || found == "" || found == want {
		// Nothing to confine to. A definition read from a file, as `vela dry-run`
		// and `vela show` do, carries no namespace of its own.
		return nil
	}
	return fmt.Errorf(
