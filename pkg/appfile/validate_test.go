...
func TestHasFromSourceParams(t *testing.T) {
	tests := []struct {
		name   string
		params map[string]interface{}
		want   bool
		...
		{
			name:   "no fromSource",
			params: map[string]interface{}{"image": "nginx:1.25"},
			want:   false,
		},
		{
			name:   "top-level fromSource shorthand",
			params: map[string]interface{}{"image": map[string]interface{}{"fromSource": "img.image"}},
			want:   true,
		},
		{
			name: "fromSource map form",
			params: map[string]interface{}{
				"vpcId": map[string]interface{}{
					"fromSource": map[string]interface{}{"name": "info", "path": "vpcId", "default": ""},
				},
			},
			want: true,
		},
		{
			name: "nested fromSource in object",
			params: map[string]interface{}{
				"config": map[string]interface{}{
					"region": map[string]interface{}{"fromSource": "info.region"},
				},
			},
			want: true,
		},
		{
			name: "fromSource inside array entry",
			params: map[string]interface{}{
				"envs": []interface{}{
					map[string]interface{}{"value": map[string]interface{}{"fromSource": "info.region"}},
				},
			},
			want: true,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert.Equal(t, tt.want, hasFromSourceParams(tt.params))
		})
	}
}
