...
// worth being exact about which. currentAppRevIsNew compares only the
// Application spec unless `app.oam.dev/autoUpdate` is set on the app, so by
// default a definition edit does not roll a revision: definitions are pinned at
// first render, and tracking them is opted into. Verified on a cluster, where
// the same edit produced no new revision without the annotation and
// acme-billing-v2 with it. This test pins the hash, not the policy.
